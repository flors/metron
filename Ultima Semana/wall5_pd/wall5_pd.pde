...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.checkAddrPattern("/interior")==true) {
    /* check if the typetag is the right one. */
    t_interior= theOscMessage.get(0).floatValue();
  } 

  if (theOscMessage.checkAddrPattern("/laterales")==true) {
    /* check if the typetag is the right one. */
    t_laterales= theOscMessage.get(0).floatValue();
  }
  
    if (theOscMessage.checkAddrPattern("/apaga")==true) {
    /* check if the typetag is the right one. */
    t_apaga= theOscMessage.get(0).floatValue();
  }
